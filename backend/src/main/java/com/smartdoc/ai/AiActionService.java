...
public class AiActionService {
 ...
 private static final AiEvidenceSelector EVIDENCE=new AiEvidenceSelector(8,120);
 ...
 @Transactional public AiActionResponse execute(long userId,long documentId,AiActionRequest request){
  try(DocumentLockManager.Handle ignored=locks.acquire(documentId)){
   ...
   AiEvidenceSelector.Selection evidence=action==AiAction.ASK?EVIDENCE.select(documentId,ordered,question,contextLimit):null;
   if(evidence!=null&&evidence.getSources().isEmpty())throw new InvalidDocumentException("文档没有可用于回答的文本上下文");
   if(!request.isForce()){AiResultRecord cached=results.selectLatestByCacheKey(documentId,key);if(cached!=null)return response(cached,true,responseSources(action,cached,evidence,relevant));}
   String context=action==AiAction.ASK?evidence.getContext():SELECTED_ACTIONS.contains(action)?truncate(selectedOriginal,contextLimit):boundedContext(relevant,contextLimit);
   String source=action==AiAction.ASK?evidence.getSources().get(0).getText():SELECTED_ACTIONS.contains(action)?truncate(selectedOriginal,sourceLimit):truncate(exactText(relevant),sourceLimit);
   ...
   AiResultRecord row=new AiResultRecord();row.setDocumentId(documentId);row.setAction(action.name());row.setCacheKey(key);row.setSourcePage(action==AiAction.ASK?evidence.getSources().get(0).getPageNumber():action==AiAction.CURRENT_PAGE_SUMMARY||SELECTED_ACTIONS.contains(action)?request.getPageNumber():null);row.setSourceText(source);row.setContentMarkdown(content);row.setMode(mode);row.setModel(model);row.setCreatedAt(LocalDateTime.now());
   if(results.insert(row)!=1||row.getId()==null)throw new IllegalStateException("AI result insert failed");AiResultRecord stored=results.selectById(row.getId());if(stored==null)throw new IllegalStateException("AI result insert verification failed");return response(stored,false,responseSources(action,stored,evidence,relevant));
  }
 }
 private static AiActionResponse response(AiResultRecord row,boolean cached,List<AiActionResponse.Source> sources){AiActionResponse.Source source=sources.isEmpty()?new AiActionResponse.Source(row.getDocumentId(),row.getSourcePage(),row.getSourceText()):sources.get(0);return new AiActionResponse(row.getId(),AiAction.valueOf(row.getAction()),row.getContentMarkdown(),row.getMode(),cached,source,sources,row.getCreatedAt());}
 private static List<AiActionResponse.Source> responseSources(AiAction action,AiResultRecord row,AiEvidenceSelector.Selection evidence,List<DocumentChunkRecord> relevant){if(action==AiAction.ASK&&evidence!=null)return evidence.getSources();Integer chunkIndex=relevant==null||relevant.isEmpty()?null:relevant.get(0).getChunkIndex();return java.util.Collections.singletonList(new AiActionResponse.Source(row.getDocumentId(),row.getSourcePage(),chunkIndex,row.getSourceText(),"RELATED"));}
