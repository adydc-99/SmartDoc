...
public class VisionActionService {
    private static final int MAX_IMAGE_BYTES=8*1024*1024,MAX_QUESTION_POINTS=500,MAX_OBSERVATION_POINTS=16_000;
    ...
    private final DocumentMapper documents;private final FileStorage storage;private final VisionCacheMapper cache;private final ModelRouter router;
    ...
    public VisionActionService(DocumentMapper documents,FileStorage storage,VisionCacheMapper cache,ModelRouter router,DailyAiQuota quota,ObjectMapper json,PdfPageImageRenderer pdf,Clock clock){this.documents=documents;this.storage=storage;this.cache=cache;this.router=router;this.quota=quota;this.json=json;this.pdf=pdf;this.clock=clock;}
    ...
    private NormalizedImage image(DocumentRecord document,Integer pageNumber,MultipartFile screenshot)throws Exception{
        ...
        byte[] bytes=screenshot.getBytes();if(!hasSupportedMagic(bytes))throw new InvalidDocumentException("仅支持 PNG 或 JPEG 图片");
        BufferedImage decoded;try(InputStream in=new ByteArrayInputStream(bytes)){decoded=ImageIO.read(in);}catch(IOException e){throw new InvalidDocumentException("图片无法解码");}
    ...
    private static boolean hasSupportedMagic(byte[] b){boolean png=b.length>=8&&(b[0]&255)==0x89&&b[1]==0x50&&b[2]==0x4e&&b[3]==0x47&&b[4]==0x0d&&b[5]==0x0a&&b[6]==0x1a&&b[7]==0x0a;boolean jpeg=b.length>=3&&(b[0]&255)==0xff&&(b[1]&255)==0xd8&&(b[2]&255)==0xff;return png||jpeg;}
    ...
    private static ProviderHttpException invalid(){return new ProviderHttpException("INVALID_PROVIDER_RESPONSE",502);}
